 ...
 * Includes: case info, escalation history, rescue status timeline
 */
...
try {
    ...
    $stmt->execute();
    $caseInfo = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt->bindParam(':case_id', $case_id, PDO::PARAM_INT);
    $stmt->execute();
    $rescueStatus = $stmt->fetch(PDO::FETCH_ASSOC);
    
    // 4. Build the timeline with 5 fixed steps
    $timeline = [];
    
    // Determine which steps are completed based on case_status (case-insensitive checks)
    // If no entry in case_status table, check if case is accepted/closed from cases table
    ...
    // Check for closed status - can be from case_status table OR from cases table
    ...
    $timeline[] = [
        ...
        'description' => $acceptedCase 
            ? (($rescueStatus['rescue_center'] ?? 'Rescue Center') . ' accepted the case')
    ...
    $response = [
        ...
        'timeline' => $timeline,
        // Debug data to check what values are in case_status table
        'debug_data' => $rescueStatus ? [
            'has_case_status' => true,
            'acceptance_status' => $rescueStatus['acceptance_status'],
            'reached_location' => $rescueStatus['reached_location'],
            'spot_animal' => $rescueStatus['spot_animal'],
            'rescued_animal' => $rescueStatus['rescued_animal'],
            'status' => $rescueStatus['rescue_status']
        ] : [
            'has_case_status' => false,
            'message' => 'No entry found in case_status table for this case_id'
        ]
    ];
